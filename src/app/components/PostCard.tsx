...
import { Post, formatNumber } from "./mockData";
...
export function PostCard({ post }: PostCardProps) {
  ...
  const [votedOption, setVotedOption] = useState(post.poll?.voted);
        ...
        <div className="flex-shrink-0 relative">
          ...
          {post.user.verified && (
            <div
              ...
              style={{ background: "#7c3aed", fontSize: 8, color: "#fff" }}
            >
              ✓
            </div>
          )}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-start justify-between mb-1">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-white" style={{ fontWeight: 700, fontSize: 14 }}>{post.user.name}</span>
              <span style={{ color: "#475569", fontSize: 13 }}>@{post.user.handle}</span>
              {post.user.badge && (
                <span
              ...
              <span style={{ color: "#475569", fontSize: 13 }}>{post.timestamp}</span>
            </div>
            <button
              className="w-7 h-7 rounded-full flex items-center justify-center transition-all duration-150 opacity-0 group-hover:opacity-100"
              style={{ color: "#475569" }}
          ...
          {post.community && (
            <div className="mb-2">
              <span
                ...
                style={{
                  background: post.community.color + "20",
                  border: `1px solid ${post.community.color}40`,
          ...
          <div
            className="mb-3"
            style={{ color: "#cbd5e1", fontSize: 15, lineHeight: 1.6, whiteSpace: "pre-line" }}
          >
            {displayContent}
            {isLong && (
              <button
                onClick={e => { e.stopPropagation(); setShowFullContent(p => !p); }}
            ...
            <div className={`mb-3 rounded-xl overflow-hidden ${post.images.length > 1 ? "grid grid-cols-2 gap-0.5" : ""}`}>
              {post.images.map((img, i) => (
                <img
                  ...
                  style={{ maxHeight: post.images!.length > 1 ? 240 : 400 }}
                />
              ))}
            </div>
          )}
            ...
            <div className="mb-3 rounded-xl overflow-hidden relative" style={{ maxHeight: 340 }}>
              ...
              {post.videoDuration && (
                <div
                  ...
                  style={{ background: "rgba(0,0,0,0.7)", color: "#fff", fontSize: 12, fontWeight: 600 }}
                >
                  {post.videoDuration}
                </div>
              )}
          ...
          {post.poll && (
            ...
            >
                ...
                {post.poll.options.map(option => {
                  ...
                  return (
                    <button
                      ...
                      disabled={hasVoted}
                      className="relative w-full text-left px-4 py-2.5 rounded-lg overflow-hidden transition-all duration-200"
                      style={{
                    ...
                    >
                      ...
                      <div className="relative flex items-center justify-between">
                        <span style={{ color: voted ? "#a78bfa" : "#cbd5e1", fontWeight: voted ? 600 : 400, fontSize: 14 }}>
                          {option.text}
                        </span>
                        {hasVoted && (
              ...
              <p className="mt-2" style={{ color: "#475569", fontSize: 12 }}>
                {formatNumber(totalVotes)} votes · {post.poll.endsIn} remaining
              </p>
            </div>
          )}
          ...
          <div className="flex items-center gap-1">
            {[
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-full transition-all duration-150 group/btn"
              ...
              >
                ...
                />
                <span style={{ color: active ? activeColor : "#475569", fontSize: 13 }}>
                  {formatNumber(count)}
                </span>
              </button>
            ))}
            ...
            <button
              ...
              className="ml-auto flex items-center gap-1.5 px-3 py-1.5 rounded-full transition-all duration-150"
